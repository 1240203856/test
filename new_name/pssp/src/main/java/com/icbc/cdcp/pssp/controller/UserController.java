...
public class UserController {
    ...
    public String selectByInfo(@RequestBody String s)
    {
        ...
        ResultVo resultVo = new ResultVo();
        List<UserInfo> ret =  userService.selectByInfo(username, gender, roomNum, phone, email, grpId, depId, comId, roleId, leaderName, isDelete);
        resultVo.setSuccess(true);
        resultVo.setMessage("已显示满足当前条件的所有员工信息！");
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("data",ret);
    ...
    /*
    @GetMapping("/login")
    public String login(@PathParam("username") String username , @PathParam("password") String password, HttpServletResponse response){
        //假设数据库的用户名密码
        UserInfo userInfo = new UserInfo();
        userInfo.setUserId(1);
        userInfo.setPassword("123");
        userInfo.setUserName("kangql");
    ...
    /*
    @GetMapping("/login")
    public ResultVo login(@PathParam("user") String username, @PathParam("pass") String password)
    {
        ResultVo resultVo = new ResultVo();
    ...
    public String login(@RequestBody String s)
    {
        ...
        String username = String.valueOf( JSON.parseObject(s).getJSONArray("user"));
        String password = String.valueOf( JSON.parseObject(s).getJSONArray("pass"));
    ...
    public String login(@RequestBody String s)
    {
        ...
        if (!matcher.matches())
        {
            resultVo.setState("0000"); //邮箱格式错误
            resultVo.setSuccess(false);
            resultVo.setMessage("邮箱格式错误");
            return resultVo;
        }*/